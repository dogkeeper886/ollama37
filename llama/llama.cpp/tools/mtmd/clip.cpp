...
bool clip_image_preprocess(struct clip_ctx * ctx, const clip_image_u8 * img, struct clip_image_f32_batch * res_imgs) {
    ...
    if (clip_is_minicpmv(ctx)) {
        ...
        // NOTE(#374): the embedder is rewritten to match ollama (fine grid + AvgPool2D), which
        // fixed the low-res degeneration, but the image is still not perceived ("no image" at this
        // 16-token res; smartResize at 266 tokens degenerates). Fixed-square is the coherent,
        // graceful checkpoint until the remaining embedding bug is found via tensor inspection.
        clip_image_u8 resized_image;
