        ...
        case LLM_ARCH_QWEN35:
            {
                ...
                // hybrid layer pattern: every Nth layer is full attention, rest are DeltaNet
                uint32_t full_attn_interval = 4;
                ml.get_key(LLM_KV_FULL_ATTENTION_INTERVAL, full_attn_interval, false);

                // mark recurrent (DeltaNet) vs attention layers
                for (uint32_t i = 0; i < hparams.n_layer; ++i) {
                    // full attention layers are at positions where (i % interval) == (interval - 1)
        ...
        case LLM_ARCH_GEMMA4:
            {
                ...
                hparams.f_attention_scale = 1.0f; // gemma4: self.scaling = 1.0 (no pre-attn scaling)
            } break;
...
bool llama_model::load_tensors(llama_model_loader & ml) {
    ...
    // create tensors for the weights
    {
        ...
        switch (arch) {
            ...
            case LLM_ARCH_QWEN35:
                {
                    // DeltaNet SSM dimension calculations
                    const int64_t ssm_d_state   = hparams.ssm_d_state;
                    const int64_t ssm_d_conv    = hparams.ssm_d_conv;
                    const int64_t ssm_d_inner   = hparams.ssm_d_inner;
                    const int64_t ssm_dt_rank   = hparams.ssm_dt_rank;
                    const int64_t ssm_n_group   = hparams.ssm_n_group;
                    const int64_t head_k_dim    = ssm_d_state;
                    const int64_t head_v_dim    = ssm_d_state;
                    const int64_t n_k_heads     = ssm_n_group;
                    const int64_t n_v_heads     = ssm_dt_rank;
                    const int64_t key_dim       = head_k_dim * n_k_heads;
                    const int64_t value_dim     = head_v_dim * n_v_heads;
                    // conv processes Q, K, and V (like B and C in Mamba2)
                    ...
                    for (int i = 0; i < n_layer; ++i) {
                        ...
                        // post-attention norm (serves as pre-FFN norm in this architecture)
            ...
            case LLM_ARCH_GEMMA_EMBEDDING:
                {
                    ...
                    for (int i = 0; i < n_layer; ++i) {
                        ...
                        // per-layer head dim (gemma4: SWA layers use a smaller head dim; identity for others)
...
// gemma4: like gemma3_iswa, but head dim, GQA count, and rope dim are per-layer
// (SWA layers use a smaller head dim than full-attention layers).
struct llm_build_gemma4_iswa : public llm_graph_context {
    llm_build_gemma4_iswa(const llama_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        ggml_tensor * cur;
        ggml_tensor * inpL;
...
// input dim:  [x, y, c, b]
// output dim: [x, y, 1, b]
static ggml_tensor * qwen35_get_slice_2d(ggml_context * ctx0, ggml_tensor * t, int64_t c) {
    return ggml_view_4d(ctx0, t, t->ne[0], t->ne[1], 1, t->ne[3],
        t->nb[1], t->nb[2], t->nb[3], t->nb[2] * c);
}

#define QWEN35_CHUNK_SIZE 64
...
struct llm_build_qwen35 : public llm_graph_context_mamba {
    ...
    llm_build_qwen35(const llama_model & model, const llm_graph_params & params) :
        ...
        ggml_tensor * causal_mask =
            ggml_tri(ctx0, ggml_fill(ctx0, ggml_new_tensor_2d(ctx0, GGML_TYPE_F32, QWEN35_CHUNK_SIZE, QWEN35_CHUNK_SIZE), 1.0f),
                        GGML_TRI_TYPE_LOWER);

        ggml_tensor * identity = ggml_diag(ctx0, ggml_fill(ctx0, ggml_new_tensor_1d(ctx0, GGML_TYPE_F32, QWEN35_CHUNK_SIZE), 1.0f));
    ...
    // === DeltaNet chunked path (prefill, n_seq_tokens > 1) ===
    std::pair<ggml_tensor *, ggml_tensor *> build_delta_net_chunking(
            ggml_tensor * q, ggml_tensor * k, ggml_tensor * v, ggml_tensor * g,
            ggml_tensor * beta, ggml_tensor * state,
            ggml_tensor * causal_mask, ggml_tensor * identity, ggml_tensor * diag_mask,
            int il) {
        ...
        const int64_t chunk_size = QWEN35_CHUNK_SIZE;
        ...
        // get last element in g_cumsum along chunk_size dimension
        ...
        for (int64_t chunk = 0; chunk < n_chunks; chunk++) {
            ggml_tensor * q_chunk = qwen35_get_slice_2d(ctx0, q, chunk);
            ggml_tensor * v_chunk = qwen35_get_slice_2d(ctx0, v, chunk);
            ggml_tensor * gexp_chunk = qwen35_get_slice_2d(ctx0, gexp, chunk);
            ggml_tensor * k_cumdecay_chunk = qwen35_get_slice_2d(ctx0, k_cumdecay, chunk);
            ggml_tensor * attn_chunk = qwen35_get_slice_2d(ctx0, attn_kq, chunk);
            ...
            ggml_tensor * k_gdiff_t = qwen35_get_slice_2d(ctx0, key_gdiff_t, chunk);
            ggml_tensor * kgdmulvnew = ggml_mul_mat(ctx0, v_new_t, k_gdiff_t);

            ggml_tensor * gexp_last_chunk = ggml_cont(ctx0, qwen35_get_slice_2d(ctx0, g_last_exp, chunk));
    ...
    // === DeltaNet autoregressive path (decode, single token) ===
    std::pair<ggml_tensor *, ggml_tensor *> build_delta_net_autoregressive(
            ggml_tensor * q, ggml_tensor * k, ggml_tensor * v, ggml_tensor * g,
            ggml_tensor * beta, ggml_tensor * state, int il) {
        const int64_t S_k      = q->ne[0];
        const int64_t H_k      = q->ne[1];
        const int64_t n_tokens = q->ne[2];
        const int64_t n_seqs   = q->ne[3];
        ...
        // state = state + k.unsqueeze(-1) * delta
    ...
    ggml_tensor * build_norm_gated(ggml_tensor * input, ggml_tensor * weights, ggml_tensor * gate, int layer) {
    ...
    ggml_tensor * build_layer_attn(
            llm_graph_input_attn_kv * inp, ggml_tensor * cur,
            ggml_tensor * inp_pos, int * sections, int il) {
        const int64_t n_embd_head = hparams.n_embd_head_v;
        GGML_ASSERT(n_embd_head == hparams.n_embd_head_k);
    ...
    ggml_tensor * build_layer_attn_linear(
            llm_graph_input_rs * inp, ggml_tensor * cur,
            ggml_tensor * causal_mask, ggml_tensor * identity, ggml_tensor * diag_mask,
            int il) {
        const auto * mctx_cur = inp->mctx;
        ...
        int64_t nb1_qkv = ggml_row_size(conv_output->type, qkv_dim);

        ggml_tensor * q_conv =
            ggml_view_2d(ctx0, conv_output, head_k_dim * num_k_heads, n_seq_tokens * n_seqs, nb1_qkv, 0);
        cb(q_conv, "q_conv", il);
        ggml_tensor * k_conv =
            ggml_view_2d(ctx0, conv_output, head_k_dim * num_k_heads, n_seq_tokens * n_seqs, nb1_qkv,
                         head_k_dim * num_k_heads * ggml_element_size(conv_output));
        cb(k_conv, "k_conv", il);
        ggml_tensor * v_conv =
            ggml_view_2d(ctx0, conv_output, head_v_dim * num_v_heads, n_seq_tokens * n_seqs, nb1_qkv,
        ...
        // Choose DeltaNet path based on sequence length
        ...
        // Gated normalization
        ggml_tensor * attn_out_2d = ggml_reshape_2d(ctx0, output, head_v_dim, num_v_heads * n_seq_tokens * n_seqs);
