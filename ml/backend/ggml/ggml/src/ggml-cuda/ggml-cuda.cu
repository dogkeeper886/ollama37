...
static void ggml_cuda_op_mul_mat_cublas(
    ...
    // BFloat16 (BF16) Support by Architecture:
    // - NVIDIA: Ampere+ (cc >= 8.0) - RTX 3000+, A100, H100
    // - AMD: All supported AMD GPUs (GCN4+, CDNA, RDNA)
    // - Moore Threads: QY2+ GPUs
    //
    // Pre-Ampere NVIDIA GPUs do NOT support BF16:
    // - Tesla K80 (cc=3.7 Kepler), GTX 1080 (cc=6.1 Pascal), RTX 2080 (cc=7.5 Turing)
    //
    // Using BF16 on unsupported GPUs causes CUBLAS_STATUS_ARCH_MISMATCH when calling
    // cublasGemmEx with CUDA_R_16BF and CUBLAS_GEMM_DEFAULT_TENSOR_OP.
    //
    // FIX: Check for Ampere or newer on NVIDIA GPUs
    const bool supports_bf16 = (GGML_CUDA_CC_IS_NVIDIA(cc) && cc >= GGML_CUDA_CC_AMPERE) ||
